...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'q.shape={q.shape},k.shape={k.shape}')   #q.shape=torch.Size([1, 65536, 512]),k.shape=torch.Size([1, 512, 65536])


        with torch.amp.autocast('cuda'):
            w_ = torch.bmm(q,k)     # b,hw,hw    w[b,i,j]=sum_c q[b,i,c]k[b,c,j]###显存溢出...
            # print(f'w.shape={w.shape}')
            w_ = w_ * (int(c)**(-0.5))
            # print(f'w_.shape={w_.shape}')       # ([1, 65536, 65536])
            w_ = torch.nn.functional.softmax(w_, dim=2)
            # print(f'After softmax w_.shape={w_.shape}') #([1, 65536, 65536])
            torch.cuda.empty_cache()

            # attend to values
            v = v.reshape(b,c,h*w)
            w_ = w_.permute(0,2,1)   # b,hw,hw (first hw of k, second of q)
            v = v.half()  # Ensure v is of type float16
            w_ = w_.half()
...
def make_attn(in_channels, attn_type="vanilla", attn_kwargs=None):
    ...
    if attn_type == "vanilla":
        ...
        return AttnBlock(in_channels)###############这个会显存溢出
